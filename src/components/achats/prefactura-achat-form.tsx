...
// Esquema de validación para las líneas de prefactura
const prefacturaItemSchema = z.object({
...
// Esquema de validación para la prefactura completa
const prefacturaSchema = z.object({
  ...
  items: z.array(prefacturaItemSchema).min(1, "Agrega al menos un concepto"),
});

// Tipo para las props del formulario
interface PrefacturaAchatFormProps {
  initialData?: any;
  onSubmit: (data: any) => Promise<void>;
  isEditing?: boolean;
}

export const PrefacturaAchatForm: React.FC<PrefacturaAchatFormProps> = ({
  initialData,
  onSubmit,
  isEditing = false,
}) => {
  ...
  const form = useForm<z.infer<typeof prefacturaSchema>>({
    resolver: zodResolver(prefacturaSchema),
  ...
  // Configurar fieldArray para los items de la prefactura
  ...
  const handleSubmit = async (values: z.infer<typeof prefacturaSchema>) => {
    try {
      setIsLoading(true);
      ...
      toast({
        title: "Éxito",
        description: `Préfacture ${isEditing ? "actualizada" : "creada"} correctamente`,
      });
      
      // Redirigir a la lista de prefacturas
      ...
      toast({
        ...
        description:
          ...
          `Error al ${isEditing ? "actualizar" : "crear"} la préfacture`,
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  // Agregar nuevo item a la prefactura
  const addItem = () => {
    append({
      descripcion: "",
      cantidad: 1,
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Numéro de Préfacture</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder="PREF-ACHAT-2024-001"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">Détail de la préfacture</h3>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder="Notes additionnelles pour cette préfacture"
                      rows={4}
                    />
                  </FormControl>
                  <FormMessage />
