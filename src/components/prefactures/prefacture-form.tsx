...
import { PrefactureItemsForm } from "./prefacture-items-form";
...
type PrefacturaFormData = z.infer<typeof prefacturaSchema>;
...
export function PrefactureForm({
  initialData,
  onSubmit,
  isLoading = false,
  submitLabel = "Guardar",
  ...
  const form = useForm<PrefacturaFormData>({
  ...
  const handleSubmit = async (data: PrefacturaFormData) => {
    if (items.length === 0) {
      alert("Debe agregar al menos una línea de detalle");
      return;
    }

    const prefacturaCompleta = {
      ...data,
      lineasDetalle: items,
    ...
    try {
      await onSubmit(prefacturaCompleta);
    } catch (error) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Número de Prefactura *</FormLabel>
                  <FormControl>
                    <Input placeholder="PRE-2024-001" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        {/* Líneas de Detalle */}
        <PrefactureItemsForm items={items} onChange={setItems} />
